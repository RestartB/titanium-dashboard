<script lang="ts">
  ...
  let {
    ...
    criterion: AutomodCriterionSchema;
    limit: number;
...
<div class="flex items-center justify-center gap-2 rounded-lg border-2 border-zinc-600 bg-zinc-700 p-1 px-2 text-base">
  {#if criterion.type !== 'discord_dice_roll' && criterion.type !== 'malicious_link' && criterion.type !== 'phishing_link' && criterion.type !== 'nsfw_link'}
