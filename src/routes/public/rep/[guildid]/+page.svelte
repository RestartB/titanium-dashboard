<script lang="ts">
  ...
  import { getRepLeaderboard } from '$lib/remote/rep.remote';
  ...
  let leaderboardQuery = $derived(
    data.serverBranding && data.enabled && !data.loginRequired && !data.noAccess
      ? getRepLeaderboard({ guildId: data.serverBranding.id, limit: 100, offset: 100 * currentPage - 100 })
      : null
  );
...
{#snippet lbRow(
  ...
  <div class="flex w-full items-center gap-2" {id}>
    ...
    <p class="max-w-14 min-w-14 text-center font-bold">{rep}</p>
  </div>
{/snippet}
